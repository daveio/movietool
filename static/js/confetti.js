...
class Confetti {
  ...
  animate() {
    ...
    this.particles.forEach((particle) => {
      const top = parseFloat(particle.element.style.top);
      const left = parseFloat(particle.element.style.left);
      ...
      particle.element.style.opacity = Math.max(
        0,
        parseFloat(particle.element.style.opacity) - particle.opacitySpeed,
      );
